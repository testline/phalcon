...
class CategoriesController extends \Phalcon\Mvc\Controller {
    ...
    public function indexAction() {
        ...
        $this->view->treeData = $this->getCategoriesForTree();
    }
...
//                    $category->delete();
    ...
    // Functions for generating categories tree
    // Returns data for categories tree
    public function getCategoriesForTree() {
        // Get all categories
        $this->categories = $this->di->get('db')->query("SELECT * FROM categories ORDER BY level, name_ru")->fetchAll();
        $this->treeData = array();
    ...
    public function generateJsonForSubtree($node) {
        ...
        $out->target = '_self';
        $out->url = '#categories/edit/' . $node['id'];
    ...
    // END Functions for generating categories tree
