...
class QForm {
    ...
    public $css_ignore_class = 'ignore';//form-control
...
//                $placeholders.='$("#' . $this->getFieldId($field_shortname) . '").defaultValue("' . addslashes($val) . '");';
        ...
        success:  function(responseText, statusText, xhr, $form){
               ...
               //$("#loadingOverlay").hide(); ajax success from refresh page will hide it
            }
    ...
    public function render($elementName, $title) {
        ?>
        <div class="form-group">
            <label class="col-sm-2 control-label" for="<?= $this->form_id ?>_<?= $elementName ?>"><?= $title ?></label>
            <div class="col-sm-10">
                <?= $this->d($elementName) ?>
            </div>
        </div>
    ...
    public function showSubmitButton($title = 'Сохранить') {
        ?>
        <div class="form-group">
        ...
        <?
        //return '<input type="submit" class="btn btn-default"  value="' . $title . '">';
    }
...
//        $TEXTS = &$CONTROLTEXTS;
//    } else global $TEXTS;
    $TEXTS = Phalcon\DI::getDefault()->get('texts')->texts;
